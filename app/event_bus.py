...
def emit(
    ...
    *,
    ...
    if commit:
        ...
        session.flush()
    enriched = {**payload, "ride_id": ride_id, "event_type": event_type}
    _to_posthog_async(event_type, payload, ride_id)
    _to_platform_queue(event_type, enriched, ride_id)
    return ev
...
def _to_platform_queue(event_type: str, payload: dict, ride_id: int | None) -> None:
    ...
    try:
        ...
        if "phase" in event_type:
            enqueue_job(
                "ride.phase_changed",
                extra={**payload, "ride_id": ride_id, "event_type": event_type},
            )
        else:
            enqueue_job(
                event_type if "." in event_type else f"ride.{event_type}",
                extra={**payload, "ride_id": ride_id, "event_type": event_type},
            )
